...
隔离出普适的 VED 单变量效应。该门禁覆盖 Copilot 回答、方案创建和方案编辑；历史 Copilot
方案如果违反契约，
...
生成，模型只生成拟议变量操作、拟议测量、拟议控制和设计风险。结构化解析、来源标签或
...
`feedback_id`、scope、`finding_id`、`claim_id`、`finding_fingerprint`、`review_status`、
...
`curation_id`、scope、`finding_id`、`claim_id`、`finding_fingerprint`、专家校正字段、`note`、
...
`expert_action`、`issue_type`、`expert_note` 和 `corrected_*` 字段；浏览器可以把填写后的
...
- `decision_board_tsv`: 可选，来自 `decision_board_tsv` 下载结果的 TSV 内容。
  提供该字段时后端会按当前 goal dataset 重新补齐 acceptance gate、protocol
...
`affected_goals`。响应还包含 `readiness_summary`，用于汇总这批决策导入后会有
多少 goal 具备 training-ready、message-ready 和 protocol-ready 输入，以及是否仍有
