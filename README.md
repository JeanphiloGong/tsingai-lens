...
- Proxied API docs from frontend: http://localhost:8080/docs
...
- If a browser has a previously saved Base URL, use the UI `Base URL -> Reset` action to switch back to the recommended default.
...
```bash
docker build -t jeanphilo/tsingai-lens-backend:v0.1.0 ./backend
docker build -t jeanphilo/tsingai-lens-frontend:v0.1.0 ./frontend

docker push jeanphilo/tsingai-lens-backend:v0.1.0
docker push jeanphilo/tsingai-lens-frontend:v0.1.0
```

```bash
cp backend/.env.example backend/.env

export LENS_VERSION=v0.1.0
...
- API docs through frontend: http://localhost:8080/docs
...
- Push a tag such as `v0.1.0` to trigger `.github/workflows/release-images.yml`.
...
- 推送 `v0.1.0` 这类版本 tag 后，会自动触发 `.github/workflows/release-images.yml`。
