...
`expert_action`、`issue_type`、`expert_note` 和 `corrected_*` 字段；原始 TSV
不是 `/review-decisions/import` 的输入，必须先通过
`merge_expert_decision_board.py` 或等价流程合并为 reviewed JSONL 后再 dry-run/import。
