...
def build_research_objective_discovery_prompt(
    ...
    user_prompt = (
        ...
        "- `candidate_materials` and `candidate_processes` are bounded scope hints.\n"
        ...
        "then construct one question with separate exact role regions.\n"
        "5. Add a seed document only when that skim supports every returned variable "
        "and outcome. Reviews and background mentions are not direct seeds.\n"
        ...
        "- Put fixed material, process, sample, and test scope in `constraints`. Every "
        ...
        "density?\",\"variables\":[\"laser power\"],\"outcomes\":[\"relative "
