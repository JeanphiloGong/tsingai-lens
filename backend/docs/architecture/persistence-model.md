...
The application model is deliberately smaller than the relational schema:
...
`Document` owns its upload identity, filename, storage key, content hash,
media type, status, size, and creation time. There are no separate application
membership, version, file, import-manifest, or intake-handoff record types.
PostgreSQL privately retains normalized
content, version, membership, object, and uploaded-document rows because
Source and Evidence tables need stable foreign keys. These SQL rows are an
adapter implementation detail and do not define additional domain objects.
...
| Collection documents and private relational lineage | PostgreSQL `stored_objects`, `collection_files`, `documents`, `document_versions`, and `collection_documents` | No | PostgreSQL plus object storage |
...
The collection aggregate is one direct application boundary. Adding a
`Document` writes its immutable bytes and, in one repository transaction,
creates the private SQL rows needed for content identity, collection
membership, stored-object lineage, and ordering. Identical content may share
one internal immutable version while each collection retains its own object
replica, storage key, and authorization boundary. `FileCollectionWorkspace`
owns collection directories and scratch/output paths only.
`FileObjectStore` owns immutable input bytes by validated storage key and
SHA-256. No maintained runtime or export path reads a JSON manifest or scans
an input directory as collection authority.
...
The ignored local `backend/data/lens.sqlite` snapshot and its legacy Goal
identity data were intentionally deleted. PostgreSQL is the only runtime
persistence backend; no maintained path imports or reads the old SQLite data.
Historical SQLite table names remain only where required by immutable migration
history and are not a supported data or API contract.
...
- Application `Document` is the current document contained by one `Collection`.
  Its `document_id` is the identity used by HTTP and application services.
- PostgreSQL privately derives canonical content, version, and membership IDs
  from validated content SHA-256 so Source and Evidence foreign keys remain
  stable. These IDs do not appear as domain records or public resources.
- Private content identity does not merge different bytes by filename, DOI, or
...
| Private collection membership | `collection_document_id` | References collection, canonical content identity, and its exact version; the collection/content pair is unique. |
| Private content and version | `document_id`, `document_version_id` | Version references canonical content identity; SHA-256 is unique and immutable. |
| Stored object and uploaded document row | `object_id`, `file_id` | Each collection-scoped object replica references one private version; each uploaded-document row references one object and membership in the same collection. `file_id` is an internal legacy SQL column containing the application `document_id`. |
