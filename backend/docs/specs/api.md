...
decision_template | agent_review_prompt_jsonl | review_packet`，默认 `json`。`json` 返回完整数据集
...
`format=json | jsonl | messages_jsonl | training_jsonl | review_jsonl |
decision_template | agent_review_prompt_jsonl | review_packet`。返回 envelope 使用 `scope_type=collection`，
