...
def search_protocol_steps(base_dir: Path, query: str, limit: int = 10, paper_id: str | None = None) -> dict[str, Any]:
    ...
    scored.sort(key=lambda item: (float(item["score"]), float(item.get("confidence_score") or 0.0)), reverse=True)
    items = scored[:limit]
    return {
