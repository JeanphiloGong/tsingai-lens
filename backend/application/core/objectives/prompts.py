...
def build_research_objective_discovery_prompt(
    ...
    user_prompt = (
        "TASK MODEL\n"
        "Word one or more concise research objectives for a bounded batch of "
        "paper-study relationships that the backend has already determined may form "
        "one scientifically compatible group. This is objective wording, not study "
        "extraction, compatibility grouping, evidence synthesis, or inventory pruning.\n\n"
        "INPUT SCHEMA\n"
        "- `collection_id` identifies the collection and must never be copied to output.\n"
        "- `paper_relationships` contains complete backend-selected records. Each record "
        "has document metadata, one study with its design/context, and exactly one "
        "relationship with a stable `relationship_id`, the complete jointly varied "
        "factor set, one outcome, Source references, and confidence.\n"
        "- `evidence_density`, `paper_confidence`, and `warnings` describe screening "
        "quality, not scientific result strength.\n\n"
        f"Input JSON:\n{json.dumps(payload, ensure_ascii=False, separators=(',', ':'))}\n\n"
        "DECISION PROCESS\n"
        "1. Treat the supplied batch boundary as authoritative; do not move a record "
        "to another group or introduce relationships absent from this request.\n"
        "2. Choose exact varied-factor and outcome labels from the supplied "
        "relationships and phrase a question with separate variable and outcome roles.\n"
        "3. Include every relationship that supports those exact axes in one "
        "objective's `source_relationship_ids`.\n"
        "4. Split the batch into multiple objectives only when its relationships have "
        "different variable-to-outcome axes. Every record must occur in exactly one "
        "objective; the backend, not this task, decides whether a relationship is "
        "eligible for objective discovery.\n\n"
        "HARD RULES\n"
        "- The top-level object must contain exactly `objectives`. Every input "
        "relationship_id must occur exactly once across objective "
        "`source_relationship_ids`. Never invent, repeat, omit, or reject an id.\n"
        "- Preserve the complete jointly varied factor set. Never word a joint-factor "
        "relationship as an isolated single-factor effect.\n"
        "- Each objective has exactly one outcome and factors supported together by "
        "every linked relationship. Prefer 'How does/do <variables joined with and> "
        "affect <outcome>?'.\n"
        ...
        "- Return only labels and ids present in the input. The backend derives shared "
        "material, constraints, comparator, seed documents, and final lineage; do not "
        "invent those values. Return empty mechanisms and constraints.\n"
        "- Do not reassess study eligibility or compatibility; the input batch is the "
        "backend-approved scientific inventory for this wording request.\n\n"
        "BOUNDARY EXAMPLES\n"
        "- Two compatible current-work relationships both link laser power to relative "
        "density. Return one objective with both exact relationship ids.\n"
        "- One study links power and speed jointly to porosity. Return variables "
        "['power','speed']; never return either variable alone.\n"
        "- One study has density and porosity outcomes represented as two relationship "
        "records. Return two objectives, one per outcome, and account for each id once.\n"
        "- A supplied relationship may not be discarded because it has low confidence; "
        "confidence describes extraction certainty, while backend filtering happens "
        "before this request.\n\n"
        ...
        "commentary, or field-assignment explanations. A non-empty input requires at "
        "least one objective because every supplied relationship must be assigned."
    )
    system_prompt = (
        "Build concise research-objective questions for literature comparison using "
        "only the supplied compatible study relationships. Return one compact JSON object immediately. Do not "
