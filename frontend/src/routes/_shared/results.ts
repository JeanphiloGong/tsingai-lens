...
function normalizeListItem(value: unknown): ResultListItem | null {
	...
	return {
		...
		document_title: String(record.document_title ?? record.document_name ?? record.document_id ?? result_id).trim(),
...
function normalizeDocument(value: unknown, fallbackResultId: string): ResultDocument {
	...
	return {
		...
		title:
			String(record?.title ?? record?.document_title ?? record?.document_id ?? fallbackResultId).trim() ||
			fallbackResultId,
...
function normalizeActions(value: unknown, collectionId: string, fallbackDocumentId: string): ResultActions {
	const record = asRecord(value);
	return {
		open_document:
...
function normalizeListResponse(value: unknown, collectionId: string): ResultListResponse {
	...
	const items = Array.isArray(record.items)
		? record.items.map((item) => normalizeListItem(item)).filter((item): item is ResultListItem => item !== null)
		: [];
...
function filterFixtureItems(items: ResultListItem[], query: ResultsQuery) {
	return items.filter((item) => {
		if (query.material_system_normalized && item.material_label !== query.material_system_normalized) {
			return false;
		}
		if (query.property_normalized && item.property !== query.property_normalized) {
			return false;
		}
		if (query.test_condition_normalized && item.test_condition !== query.test_condition_normalized) {
			return false;
		}
...
function buildFixtureDetail(collectionId: string, resultId: string): ResultDetail {
	const listItem = buildFixtureList(collectionId).items.find((item) => item.result_id === resultId) ?? buildFixtureList(collectionId).items[0];
	return {
		...
		assessment: {
			...
			warnings: listItem.comparability_status === 'limited' ? ['Baseline alignment is incomplete.'] : [],
