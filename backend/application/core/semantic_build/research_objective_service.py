...
_OBJECTIVE_COMPARISON_SELECTION_TEXT_CHARS = 1800
_OBJECTIVE_COMPARISON_SELECTION_CANDIDATE_LIMIT = 180
...
class ResearchObjectiveService:
    ...
    def _select_objective_pairwise_comparison_units(
        self,
        *,
        extractor: CoreLLMStructuredExtractor,
        collection_id: str,
        units: tuple[ObjectiveEvidenceUnit, ...],
        comparison_units: tuple[ObjectiveEvidenceUnit, ...],
        objective_contexts: tuple[ObjectiveContext, ...],
        blocks_by_document_id: dict[str, list[Any]],
    ) -> tuple[ObjectiveEvidenceUnit, ...]:
        ...
        candidates_by_objective: dict[str, list[ObjectiveEvidenceUnit]] = {}
        for unit in comparison_units:
            candidates_by_objective.setdefault(unit.objective_id, []).append(unit)

        selected_units: list[ObjectiveEvidenceUnit] = []
        for objective_id, candidates in candidates_by_objective.items():
            if len(candidates) > _OBJECTIVE_COMPARISON_SELECTION_CANDIDATE_LIMIT:
                logger.warning(
                    "Research objective pairwise comparison selection skipped collection_id=%s objective_id=%s candidate_count=%s candidate_limit=%s",
                    collection_id,
                    objective_id,
                    len(candidates),
                    _OBJECTIVE_COMPARISON_SELECTION_CANDIDATE_LIMIT,
                )
                continue
            payload = self._objective_comparison_selection_payload(
                objective_context=context_by_objective_id.get(objective_id),
                units=tuple(
                    unit
                    for unit in units
            ...
            selection = extractor.select_objective_comparisons(payload)
            candidate_by_id = {
            ...
            selected_ids = [
                evidence_unit_id
                for evidence_unit_id in selection.selected_evidence_unit_ids
                if evidence_unit_id in candidate_by_id
            ]
            if not selected_ids:
                logger.info(
                    "Research objective pairwise comparison selection returned no supported candidates objective_id=%s candidate_count=%s rejected_candidate_count=%s",
                    objective_id,
                    len(candidates),
                    len(selection.rejected_evidence_unit_ids),
                )
                continue
            selected_units.extend(candidate_by_id[unit_id] for unit_id in selected_ids)
        return tuple(selected_units)

    def _objective_comparison_selection_payload(
        self,
        *,
        objective_context: ObjectiveContext | None,
        units: tuple[ObjectiveEvidenceUnit, ...],
        candidates: tuple[ObjectiveEvidenceUnit, ...],
        blocks_by_document_id: dict[str, list[Any]],
    ) -> dict[str, Any]:
        text_source_refs = {
            ...
            if source_ref.get("source_kind") == "text_window"
        }
        return {
            "objective_context": (
                objective_context.to_record()
            ...
            "author_evidence_units": [
                self._objective_logic_unit_reference(unit)
                for unit in units
                if any(
                    source_ref.get("source_kind") == "text_window"
                    for source_ref in unit.source_refs
                )
            ],
            "author_texts": self._objective_comparison_source_texts(
                text_source_refs=text_source_refs,
                blocks_by_document_id=blocks_by_document_id,
            ),
            "candidate_comparisons": [
                self._objective_candidate_comparison_payload(unit)
                for unit in candidates
            ],
        }

    def _objective_comparison_source_texts(
        self,
        *,
        text_source_refs: set[str],
        blocks_by_document_id: dict[str, list[Any]],
    ) -> list[dict[str, Any]]:
        source_texts: list[dict[str, Any]] = []
        for blocks in blocks_by_document_id.values():
            for block in blocks:
                block_id = str(getattr(block, "block_id", "") or "")
                if block_id not in text_source_refs:
                    continue
                ...
                source_texts.append(
                    {
                        ...
                        "text": text[:_OBJECTIVE_COMPARISON_SELECTION_TEXT_CHARS],
                    }
                )
        return source_texts

    def _objective_candidate_comparison_payload(
        self,
        unit: ObjectiveEvidenceUnit,
    ) -> dict[str, Any]:
        return {
            ...
            "baseline": {
                "sample_context": dict(
                    unit.baseline_context.get("sample_context", {})
                ),
                "process_context": dict(
                    unit.baseline_context.get("process_context", {})
                ),
                "value": unit.baseline_context.get("value"),
                "source_value_text": unit.baseline_context.get("source_value_text"),
            },
            "unit": unit.unit,
        }
        ...
        *,
        ...
        for column, role in route.column_roles.items():
            ...
            if (
                ...
                and role_label
                and self._property_axis_matches_any(
                    role_label,
                    objective_context.target_property_axes,
                )
            ):
