...
`feedback_id`、scope、`finding_id`、`claim_id`、`review_status`、
`issue_type`、`note`、`reviewer` 和 `created_at`。该资源用于沉淀专家评价数据，
不改变 `understanding` artifact 本身。

`curations` 用于保存专家校正后的 finding 副本，不覆盖系统生成的
`understanding` artifact。`POST` 请求体包含：

- `scope_type`、`scope_id`、`finding_id`，以及可选 `claim_id`
- `curated_claim_type`：
...
`curation_id`、scope、`finding_id`、`claim_id`、专家校正字段、`note`、
`reviewer` 和 `updated_at`。同一 collection/scope/finding 的 curation 使用稳定
...
可选 `format=json | jsonl | messages_jsonl | review_jsonl | decision_template | review_packet`，默认 `json`。`json` 返回完整数据集
...
导入模板，保留 `action`、`issue_type`、`expert_note`、`suggested_target` 和
`curated_evidence_ref_ids`，适合专家或复核 agent 编辑后直接交给导入脚本预检。
...
响应包含 `status`、`errors`、`warnings`、`review_progress` 和
`affected_goals`。`review_progress.ready_to_write=false` 表示模板仍没有可写入
的人工决策。该接口不会把未复核 AI 候选自动提升为 gold/training-ready。
...
`format=json | jsonl | messages_jsonl | review_jsonl | decision_template | review_packet`。返回 envelope 使用 `scope_type=collection`，
...
`dataset_use_status=review_candidate` 样本的复核模板行；`review_packet` 只输出
这些待复核样本的人类可读文本复核包。
