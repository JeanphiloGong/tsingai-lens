...
class ObjectiveContext:
    """Bounded transient lens used while analyzing one Objective."""
    ...
    def from_mapping(cls, payload: Mapping[str, Any]) -> "ObjectiveContext":
        ...
        evidence_lens = _mapping(payload.get("objective_evidence_lens"))
        evidence_lens.setdefault(
            "variable_process_axes", list(variable_process_axes)
        )
        evidence_lens.setdefault("context_axes", list(process_context_axes))
        evidence_lens.setdefault("target_outcome_axes", list(target_property_axes))
        evidence_lens.setdefault("mediator_axes", [])
        return cls(
            objective_id=_text(payload.get("objective_id")) or "",
            question=_text(payload.get("question")) or "",
            material_scope=normalize_objective_terms(payload.get("material_scope")),
            variable_process_axes=variable_process_axes,
            process_context_axes=process_context_axes,
            ...
            objective_evidence_lens=evidence_lens,
            routing_hints=_mapping_tuple(payload.get("routing_hints")),
            extraction_guidance=_mapping(payload.get("extraction_guidance")),
            confidence=normalize_objective_confidence(payload.get("confidence")),
        )
