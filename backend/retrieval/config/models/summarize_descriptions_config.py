...
class SummarizeDescriptionsConfig(BaseModel):
    ...
    model_id: str = Field(
        ...
        default=retrieval_config_defaults.summarize_descriptions.model_id,
    )
    prompt: str | None = Field(
        description="The description summarization prompt to use.",
        default=retrieval_config_defaults.summarize_descriptions.prompt,
    )
    max_length: int = Field(
        description="The description summarization maximum length.",
        default=retrieval_config_defaults.summarize_descriptions.max_length,
    )
    max_input_tokens: int = Field(
        description="Maximum tokens to submit from the input entity descriptions.",
        default=retrieval_config_defaults.summarize_descriptions.max_input_tokens,
    )
    strategy: dict | None = Field(
        description="The override strategy to use.",
        default=retrieval_config_defaults.summarize_descriptions.strategy,
    )
    ...
    def resolved_strategy(
        ...
        from retrieval.index.operations.summarize_descriptions.summarize_descriptions import (
