...
The current runtime uses PostgreSQL for mutable product records, local object
storage for immutable uploaded and generated bytes, and SQLite only for the
remaining evaluation gold, prediction, and run records. This table records the
boundary so later migrations do not accidentally preserve legacy ownership.
...
| Evaluation gold sets, prediction snapshots, and runs | `lens.sqlite` evaluation tables | No | PostgreSQL |
