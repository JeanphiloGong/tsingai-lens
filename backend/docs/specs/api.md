...
可选 `format=json | jsonl | messages_jsonl | review_jsonl | review_packet`，默认 `json`。`json` 返回完整数据集
...
`protocol_readiness`。`review_packet` 返回 `text/plain` 人工可读复核包，包含同一批
`review_candidate` 的 statement、变量/结果/方向、推荐动作、复核原因、protocol
readiness、证据 quote 和打开来源的链接，适合专家先快速浏览再决定是否导入
`review_jsonl`。`protocol_readiness` 给出 `status`、`missing`、
...
`format=json | jsonl | messages_jsonl | review_jsonl | review_packet`。返回 envelope 使用 `scope_type=collection`，
