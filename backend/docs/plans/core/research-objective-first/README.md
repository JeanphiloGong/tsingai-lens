# Research Objective First

This topic family records the Core plan for moving semantic extraction from a
paper-wide or material-wide scan into objective-shaped research workspaces.

The family keeps multiple live pages because this local subject now needs:

- a parent flow plan that explains why Core first discovers multiple research
  objectives from a collection
- a domain-model slice that adds the pure Core records before service and
  prompt work
- an objective-context extraction slice that threads the selected objective
  into paper-fact routing and extraction

This is a Core semantic-build topic family. It does not change public API
routes, frontend contracts, Source artifact production, or downstream report
ownership by itself.

## Reading Order

- [`target-centric-collection-extraction-plan.md`](target-centric-collection-extraction-plan.md)
  Parent direction for discovering multiple collection research objectives,
  running independent objective processes, and extracting objective-scoped
  facts
- [`research-objective-domain-model-plan.md`](research-objective-domain-model-plan.md)
  First implementation slice for adding pure Core objective records before
  LLM services, persistence expansion, or public surfaces
- [`objective-context-targeted-extraction-plan.md`](objective-context-targeted-extraction-plan.md)
  Concrete implementation slice for using persisted objective context during
  paper-fact routing, prompts, and deterministic binding

## Related Docs

- [`../core-parsing-quality-hardening-plan.md`](../core-parsing-quality-hardening-plan.md)
  Parent Core quality wave that this objective-first route sharpens
- [`../../../../application/core/semantic_build/README.md`](../../../../application/core/semantic_build/README.md)
  Owning Core semantic-build package entry point
- [`../../../../application/core/semantic_build/llm/docs/structured-extraction/README.md`](../../../../application/core/semantic_build/llm/docs/structured-extraction/README.md)
  Node-local structured-extraction plan family for prompt and routing work
- [`../../../../../docs/decisions/rfc-research-objective-first-product-flow.md`](../../../../../docs/decisions/rfc-research-objective-first-product-flow.md)
  Shared product-flow decision for objective-first collection analysis
