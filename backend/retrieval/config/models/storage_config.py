...
class StorageConfig(BaseModel):
    ...
    type: StorageType | str = Field(
        ...
        default=retrieval_config_defaults.storage.type,
    )
    base_dir: str = Field(
        description="The base directory for the output.",
        default=retrieval_config_defaults.storage.base_dir,
    )
    ...
    connection_string: str | None = Field(
        ...
        default=retrieval_config_defaults.storage.connection_string,
    )
    container_name: str | None = Field(
        description="The storage container name to use.",
        default=retrieval_config_defaults.storage.container_name,
    )
    storage_account_blob_url: str | None = Field(
        description="The storage account blob url to use.",
        default=retrieval_config_defaults.storage.storage_account_blob_url,
    )
    cosmosdb_account_url: str | None = Field(
        description="The cosmosdb account url to use.",
        default=retrieval_config_defaults.storage.cosmosdb_account_url,
    )
